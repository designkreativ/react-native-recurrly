...
jest.mock('expo-router', () => {
  ...
  const MockTabs = jest.fn(({ children }: { children: React.ReactNode }) => <>{children}</>);
...
function getMocks() {
  ...
  return { MockTabs: Tabs as jest.Mock, MockTabsScreen: (Tabs as any).Screen as jest.Mock, mockUseSafeAreaInsets: useSafeAreaInsets as jest.Mock };
}
...
describe('TabLayout', () => {
  ...
  it('tabBarStyle uses primary background color', () => {
    ...
    const { MockTabs } = getMocks();
    const props = MockTabs.mock.calls[0][0];
    expect(props.screenOptions.tabBarStyle.backgroundColor).toBe(colors.primary);
  });
  ...
  it('tabBarStyle has correct marginHorizontal from theme', () => {
    ...
    const { MockTabs } = getMocks();
    const props = MockTabs.mock.calls[0][0];
    expect(props.screenOptions.tabBarStyle.marginHorizontal).toBe(tabBar.horizontalInset);
  });
  ...
  it('tabBarStyle bottom respects large safe area insets (insets larger)', () => {
    const { mockUseSafeAreaInsets } = getMocks();
    mockUseSafeAreaInsets.mockReturnValue({ top: 50, bottom: 34, left: 0, right: 0 });
    render(<TabLayout />);
    const { MockTabs } = getMocks();
    const props = MockTabs.mock.calls[0][0];
...
describe('tabBar bottom calculation (unit)', () => {
  ...
  it('uses exact value when insets.bottom equals horizontalInset', () => {
    expect(Math.max(tabBar.horizontalInset, tabBar.horizontalInset)).toBe(tabBar.horizontalInset);
  });
});
